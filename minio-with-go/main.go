...
func downloadFromMinio(w http.ResponseWriter, r *http.Request) {
	...
	err := minioClient.FGetObject(ctx, BUCKET_NAME, filename, "C:/Backend/go-tour/minio-with-go/downloads", minio.GetObjectOptions{})

	if err != nil {
		http.Error(w, fmt.Sprintf("Unable to find file %q on the minio server", filename), http.StatusBadRequest)
		return
	}
	respondWithJson(w, http.StatusOK, "Download successful!")
}
...
func main() {
	...
	mux.HandleFunc("/download-from-minio/", downloadFromMinio)
